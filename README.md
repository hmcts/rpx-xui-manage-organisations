...
flowchart TB
  ...
  PwCross["yarn test:crossbrowser:raw\nFirefox/WebKit deployed journeys"]
...
- `yarn test:playwrightE2E:raw` is the Jenkins CNP Chromium E2E entrypoint and uses `playwright.e2e.config.ts`.
- `yarn test:crossbrowser:raw` is the Jenkins nightly Firefox/WebKit entrypoint and uses `playwright-nightly.config.ts`.
...
| Nightly cross-browser        | `PLAYWRIGHT_SKIP_INSTALL=true yarn test:crossbrowser:raw`       |
| Smoke                        | `yarn test:playwright:smoke`                                    |
...
PLAYWRIGHT_SKIP_INSTALL=true yarn test:crossbrowser:raw
```
...
FUNCTIONAL_TESTS_WORKERS=6 PLAYWRIGHT_SKIP_INSTALL=true yarn test:crossbrowser:raw
```
...
Required values are `TEST_USER1_EMAIL`, `TEST_USER1_PASSWORD`, `TEST_ROO_EMAIL`, and `TEST_ROO_PASSWORD`.
