...
export async function getAccount(accountId: string): Promise<PaymentAccountDto[]> {
    const response = await http.get(`${config.services.paymentApi}/accounts/${accountId}`)
    return response.data
}

export async function getPayments(accountId: string): Promise<Payments> {
    const response = await http.get(`${config.services.paymentApi}/pba-accounts/${accountId}/payments`)
    return response.data
}
