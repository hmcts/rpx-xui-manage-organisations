...
export class UserService {
  ...
  create(resource: User): Observable<number> {
    ...
    .map(data => 200); // return a createdId for now ....
  }

  createWithAny(resource: User)  {
    return this.httpClient
      .post(this.BASE_URL, resource)
      .map(data => 200); // return a createdId for now ....
  }

  createWithAxios(resource: User):Promise<AxiosResponse<any>> {
    ...
    return  axios.post(this.BASE_URL,resource,axiosConfig);
    //return response;
