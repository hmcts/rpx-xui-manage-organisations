...
test.describe('User list API contracts', { tag: '@svc-user-admin' }, () => {
  ...
  test('returns full organisation user list without requiring role expansion', async ({ apiClient }) => {
    ...
    const activeUser = users!.find(
      (user) => user.email && user.firstName && user.lastName && user.idamStatus === 'ACTIVE'
    );
    ...
    expect(activeUser!.firstName, 'Active user first name should not be empty').toBeTruthy();
    expect(activeUser!.lastName, 'Active user last name should not be empty').toBeTruthy();
  });
  ...
  test('returns selected user details by user identifier', async ({ apiClient }) => {
    ...
    expect(targetUser, 'A user identifier should be available for details lookup').toEqual(
      expect.objectContaining({
        userIdentifier: expect.any(String)
      })
    );
    const targetUserIdentifier = targetUser!.userIdentifier;
    expect(targetUserIdentifier, 'A user identifier should be available for details lookup').toBeTruthy();
    const detailsResponse = await apiClient.get<UserListResponse>(`api/user-details?userId=${targetUserIdentifier}`);
    ...
    expect(
      detailsUsers!.some((user) => user.userIdentifier === targetUserIdentifier),
