...
const defaultFunctionalOutputRoot = 'functional-output/tests/playwright-wave-a11y';
const defaultTestOutputRoot = 'test-results/playwright-wave-a11y';
...
const assertNoGrepOverrides = (args) => {
  ...
  if (override) {
    throw new Error(
      `Do not pass ${override} to test:wave-a11y:playwright. The WAVE-like runner owns the @wave-a11y grep so it remains a separate pack.`
    );
  }
};
...
const buildRunPlan = (argv = process.argv.slice(2), env = process.env) => {
  ...
  const functionalOutputRoot = resolveSafeOutputRoot(
    env.PLAYWRIGHT_WAVE_A11Y_OUTPUT_ROOT || defaultFunctionalOutputRoot,
    'PLAYWRIGHT_WAVE_A11Y_OUTPUT_ROOT'
  );
  ...
  const e2eEnv = {
    ...
    PLAYWRIGHT_JUNIT_OUTPUT: env.PLAYWRIGHT_JUNIT_OUTPUT || `${functionalOutputRoot}/playwright-wave-a11y-junit.xml`,
  ...
  return {
    ...
    runs: [
      {
        args: ['test:playwrightE2E:list', '--grep', '@wave-a11y', ...passthroughArgs],
        env: e2eEnv,
...
const runYarn = (label, args, envOverrides, env = process.env, spawn = spawnSync) => {
  console.log(`[playwright-wave-a11y] ${label}`);
  ...
  if (result.signal) {
    console.error(`[playwright-wave-a11y] ${label} terminated by signal ${result.signal}`);
    return 1;
  }
...
const main = (argv = process.argv.slice(2), env = process.env) => {
  ...
  try {
    ...
    console.error(`[playwright-wave-a11y] ${error.message}`);
    return 1;
  }
