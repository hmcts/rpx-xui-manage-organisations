...
function acquireLock() {
  try {
    console.log('[lock] try mkdir', LOCK_DIR);
    fs.mkdirSync(LOCK_DIR, { recursive: false });    // becomes the owner
    ...
    // lock dir exists; either another healthy owner or a stale lock
    const start = Date.now();
    const timeoutMs = 3000;   // wait up to 3s for owner to initialize files
    const sleep = (ms) => new Promise(r => setTimeout(r, ms));

    return (async () => {
      while (Date.now() - start < timeoutMs) {
        try {
          const n = readInt(COUNTER_FILE);        // will succeed once owner wrote it
          writeInt(COUNTER_FILE, n + 1);
          return false;                            // not the owner, but joined
        } catch {
          await sleep(50);                         // small backoff
        }
      }
      ...
      try {
        ...
        // couldn’t read pid or check; fall through and just stub
      }
      return false; // Assume active owner and continue as non-owner
    })();
  }
}
...
(async () => {
  ...
  const serverEntry = path.resolve(__dirname, '../../dist/rpx-xui-manage-organisations/api/server.bundle.js');
  ...
  if (PORT_FREE) {
    ...
    if (shouldStart && fs.existsSync(serverEntry)) {
      ...
      ssr = spawn(process.platform === 'win32' ? 'yarn.cmd' : 'yarn',
        ['node', 'dist/rpx-xui-manage-organisations/api/server.bundle.js'],
