...
module.exports = async function () {
  ...
  event.dispatcher.on(event.test.before, async function (test) {
    if (process.env.CODECEPT_WORKER_ID) {
      const p = process;
      process.on('beforeExit', () => {
        console.log(`[worker ${process.env.CODECEPT_WORKER_ID}] beforeExit`);
        console.log('Active handles:', p._getActiveHandles?.());
        console.log('Active requests:', p._getActiveRequests?.());
      });
      if (process.env.DUMP_HANDLES === '1') {
        const why = require('why-is-node-running');
        process.on('beforeExit', () => setTimeout(() => why(), 0));
      }
    }
    global.scenarioData = {}
  ...
  if (!process.env.CODECEPT_WORKER_ID) {
    // Dump open handles/requests when all tests are done, before exit
    event.dispatcher.on(event.all.result, (status) => {
      if (process.env.DUMP_HANDLES === '1') {
        try {
          const why = require('why-is-node-running');
          // 'beforeExit' is too late if you call process.exit,
          // so trigger it just before we set the exit code
          setTimeout(() => {
            console.log('\n=== Dumping active handles/requests (debug) ===');
            why(); // prints open handles with stack traces
            console.log('=== End dump ===\n');
          }, 0);
        } catch (err) {
          console.error('Could not require why-is-node-running:', err);
        }
      }

      // Decide exit code only after diagnostics
      setTimeout(() => {
        process.exit(status ? 0 : 1);
      }, process.env.DUMP_HANDLES ? 1000 : 0);
    });
  }
}
