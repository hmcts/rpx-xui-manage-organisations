...
class MailinatorService{
  ...
  async gotToInbox(useremail){
    ...
    const username = useremail.replace('@mailinator.com', '');
    ...
    const inboxLabelElement = this.mailinatorElement(by.css(this.currentInboxLabel));
  ...
  async openEmailForUser(useremail, emailElement){
    ...
    let timer = 15;
    const latestEmailElement = emailElement;
    ...
    const latestEmailEle = this.mailinatorElement(by.css(this.latestEmailRowCssSelector));
  ...
  async completeUserRegistrationFromEmail(){
    ...
    const isCookiesInfoLinkPresent = await this.acceptCookiesLink.isDisplayed();
    ...
    const activationLinkEle = this.mailinatorElement(by.css(this.activationEmaillinkCSsSelector));
    ...
    const mainWinHandle = await this.mailinatorbrowser.driver.getWindowHandle();
    ...
    const accountCreatedMessageElement = this.mailinatorElement(by.xpath('//h1[contains(text(), \'Account created\')]'));
  ...
  async getLoginVerificationEmailCode(email) {
    ...
    const messageBody = this.mailinatorElement(by.css('#msg_body'));
    ...
    const verificationCode = await this.mailinatorElement(by.css('tbody tr td p:nth-of-type(3)')).getText();
